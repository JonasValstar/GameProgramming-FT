            ...
            switch (effect.modType) {
                ...
                case ModType.statChange:
                    ...
                    switch (effect.statType) { // getting the end value
                        case StatType.critChance: description = $"{description}%\n"; break; // Percent crit chance
                        case StatType.fireDelay: description = $"{description} sec\n"; break; // seconds fire delay
                        case StatType.spreadAngle: description = $"{description} deg\n"; break; // degree angle of spread
                        case StatType.bulletsPerShot: description = $"{description} bullets\n"; break; // bullets per shot
                        case StatType.recoilForce: description = $"{description} N\n"; break; // Newton of force to recoil
                        case StatType.maxAmmo: description = $"{description} shots\n"; break; // shots before empty
                        case StatType.reloadSpeed: description = $"{description} shots/sec\n"; break; // shots that the charge reloads per second
                        case StatType.reloadDelay: description = $"{description} sec\n"; break; // seconds before reload start
                        case StatType.bulletVel: description = $"{description} m/s\n"; break; // meter per second that the bullet starts with
                        case StatType.bulletAcc: description = $"{description} m/s^2\n"; break; // meter per second squared that the bullet accelerates
                    }
                    break;
